...
import os
import gradientCalculationV2 as gradCalc
...
class GDT_GUI(tk.Tk):
    ...
    def __init__ (self, *args, **kwargs):
        ...
        resolutionLabel.grid(row=8,column=0, pady=5,sticky=tk.W)
        resolutionInput = ttk.Entry(inputFrame, textvariable=self.linearityOrder, width = 10, justify=tk.RIGHT)
        resolutionInput.grid(row=8,column=1,padx = 20)
        ...
        inductanceTextLabel = ttk.Label(outputFrame, text='Coil inductance (beta):')
    ...
    def calculateGradientWires(self):
        ...
        for idx, wireLevel in enumerate(wireLevels):
            ...
            for wire in range (np.size(wireLevel,0)):
                wirePath3D = np.stack((np.cos(wireLevel[wire][:,0])*self.coilRadius.get(), np.sin(wireLevel[wire][:,0])*self.coilRadius.get(),wireLevel[wire][:,1]*1e3),axis=1)
                if currentLevel>0:
